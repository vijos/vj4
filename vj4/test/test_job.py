...
class RecordTest(RecordTestCase):
  ...
  async def test_run(self):
    ...
    self.assertEqual(pdoc['num_submit'], 3)
    self.assertEqual(pdoc['num_accept'], 1)
    psdoc = pdoc['psdoc']
    self.assertEqual(psdoc['num_submit'], 2)
    self.assertEqual(psdoc['num_accept'], 1)
    self.assertEqual(len(psdoc['journal']), 3)
    self.assertEqual(psdoc['journal'][0]['rid'], self.rid_p1_wa_to_ac)
    self.assertEqual(psdoc['journal'][1]['rid'], self.rid_p1_ac)
    self.assertEqual(psdoc['journal'][2]['rid'], self.rid_p1_ac2)
    pdoc = await problem.get(DOMAIN_ID, self.pid1, UID2)
    self.assertEqual(pdoc['num_submit'], 3)
    self.assertEqual(pdoc['num_accept'], 1)
    psdoc = pdoc['psdoc']
    self.assertEqual(psdoc['num_submit'], 1)
    self.assertEqual(psdoc['num_accept'], 0)
    self.assertEqual(len(psdoc['journal']), 1)
    self.assertEqual(psdoc['journal'][0]['rid'], self.rid_p1u2_wa)
    ...
    self.assertEqual(psdoc['num_accept'], 1)
    self.assertEqual(len(psdoc['journal']), 4)
    self.assertEqual(psdoc['journal'][0]['rid'], self.rid_p2_wa)
    self.assertEqual(psdoc['journal'][1]['rid'], self.rid_p2_wa2)
    self.assertEqual(psdoc['journal'][2]['rid'], self.rid_p2_ac)
    self.assertEqual(psdoc['journal'][3]['rid'], self.rid_p2_wa3)
    ...
    self.assertEqual(psdoc['num_accept'], 0)
    self.assertEqual(len(psdoc['journal']), 1)
    self.assertEqual(psdoc['journal'][0]['rid'], self.rid_p2u2_wa)
    uddoc = await domain.get_user(DOMAIN_ID, UID)
    self.assertEqual(uddoc['num_submit'], 5)
